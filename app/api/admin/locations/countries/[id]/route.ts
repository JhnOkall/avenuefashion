...
/**
 * A Next.js API route handler for updating a specific country by its ID.
 ...
 * @param {string} context.params.id - The unique identifier of the country to be updated.
...
export async function PATCH(req: Request, { params }: { params: Promise<{ id: string }> }) {
  ...
  try {
    ...
    // Parses the JSON body from the incoming PATCH request (e.g., { name: 'New Name' } or { isActive: false }).
    ...
    /**
     * Finds a country document by its ID and applies the updates from the request body.
     * The `{ new: true, runValidators: true }` options ensure that the updated document is returned and schema validations are run.
     */
    const updatedCountry = await Country.findByIdAndUpdate(id, body, { new: true, runValidators: true });
    ...
    // On successful update, return a success message and the updated country data.
    ...
    // Handles potential errors, such as a duplicate key error if the country name is changed to one that already exists.
    if (error.code === 11000) {
      return NextResponse.json({ message: 'A country with this name already exists.' }, { status: 409 });
    }
    
    return NextResponse.json({ message: 'Error updating country', error: error.message }, { status: 500 });
  }
}